 ...
 * 客户端同步状态
 */
let StepTrigger = (_dec = Component.serializedName('StepTrigger'), _dec(_class = class StepTrigger extends Component {
    ...
    update() {
        for (let i = 0, len = this.gameObject.scene.onlinePlayers.length; i < len; i++) {
            ...
            if (Math.abs(pp.x - this.gameObject.position.x) <= this.props.triggerRadius && Math.abs(pp.z - this.gameObject.position.z) <= this.props.triggerRadius) {
