 ...
 * 排序控制器
 */
let SortController = (_dec = Component.serializedName('SortController'), _dec(_class = class SortController extends Component {
        ...
        if (!ENV_CLIENT) {
            ...
            for (let i = 0; i < 15; ++i) {
                ...
                switch (this.state.program) {
                    case 'bubblesort':
                        changeColor = (i === 14 - this.state.pointer_i || i === this.state.pointer_j) && this.children[i]['changed'] === false || i !== 14 - this.state.pointer_i && i !== this.state.pointer_j && this.children[i]['changed'] === true;
                        break;
                    case 'quicksort':
                    case 'insertsort':
                    case 'selectsort':
                        changeColor = (i === this.state.pointer_i || i === this.state.pointer_j) && this.children[i]['changed'] === false || i !== this.state.pointer_i && i !== this.state.pointer_j && this.children[i]['changed'] === true;
                        break;
                    default:
                        break;
                }
                if (this.children[i]['value'] != this.state.content[i] || changeColor) {
                    ...
                    this.children[i] = this.numberPool.obtain(this.state.content[i], changeColor ? !this.children[i]['changed'] : false);
    ...
    quicksort() {
        ...
        if (i == j) {
            --top;
            if (i + 1 <= t) pstack[++top] = [i + 1, t, i + 1, t];
            if (i - 1 >= h) pstack[++top] = [h, i - 1, h, i - 1];
        } else {
            if (direct) {
    ...
    selectsort() {
        ++this.state.pointer_i;
        if (this.state.pointer_i > 14) return false;
...
// 只用于 ENV_CLIENT
let NumberPool = class NumberPool {
    ...
    obtain(number, changeFlag = false) {
        let cubeMaterials = changeFlag ? [this.changedCubeMaterial, this.changedCubeMaterial, this.changedCubeMaterial, new THREE.MeshLambertMaterial({
            map: new THREE.CanvasTexture(getTextCanvas(number, '#' + this.red.toString(16)))
        }), this.changedCubeMaterial, this.changedCubeMaterial] : [this.normalCubeMaterial, this.normalCubeMaterial, this.normalCubeMaterial, new THREE.MeshLambertMaterial({
            map: new THREE.CanvasTexture(getTextCanvas(number, '#' + this.blue.toString(16)))
        }), this.normalCubeMaterial, this.normalCubeMaterial];
