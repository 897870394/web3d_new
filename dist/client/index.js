...
let App = class App extends React.Component {
    ...
    onJoinRoom(room) {
        ...
        require('../common/Resource').loadScene(sceneName).then(scene => {
            ...
            if (room.type === 1 || room.type === 2 || room.type === 4) {
                SceneHelper.createSkyBox(scene, ['images/galaxy+X.jpg', 'images/galaxy-X.jpg', 'images/galaxy+Y.jpg', 'images/galaxy-Y.jpg', 'images/galaxy-Z.jpg', 'images/galaxy+Z.jpg'], 75);
                        ...
                        loader.load('images/solar-system/' + key + '.jpg', function (texture) {
                            ...
                            if (ringConfig.has(key)) {
                                loader.load('images/solar-system/' + key + '-Ring.' + ringConfig.get(key)[2], texture => {
                                    ...
                                    let ringGeometry = new THREE.RingBufferGeometry(starsConfig.get(key).get('draw_radius') * ringConfig.get(key)[0], starsConfig.get(key).get('draw_radius') * ringConfig.get(key)[1], 30);
    ...
    render() {
        return React.createElement(
            ...
            React.createElement(
                'div',
                null,
                React.createElement(TitleBar, { user: this.state, onLogout: this.onLogout.bind(this), onLeaveRoom: this.onLeaveRoom.bind(this) }),
                this.state.login ? this.state.room ? this.state.progress < 100 ? React.createElement(
                    'p',
                    null,
                    'loading'
                ) : React.createElement(GamePanel, { game: this.game, room_type: this.room_type }) : React.createElement(RoomList, { onJoinRoom: this.onJoinRoom.bind(this) }) : React.createElement(LoginPanel, { onLogin: this.onLogin.bind(this) }),
                this.state.room ? null : React.createElement(CreateRoomButton, { onJoinRoom: this.onJoinRoom.bind(this) })
            )
        );
    }
};
